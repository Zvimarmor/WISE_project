...
class StopAtStringsCriteria(StoppingCriteria):
    def __init__(self, stop_strings: List[str], tokenizer):
        self.stop_strings = stop_strings
        self.tokenizer = tokenizer

    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor, **kwargs) -> bool:
        # Check the last 15 tokens of each sequence for stop strings.
        # This provides a safe buffer for multi-line stop strings.
        for i in range(input_ids.shape[0]):
            decoded = self.tokenizer.decode(input_ids[i][-15:], skip_special_tokens=True)
...
def generate_fast(
    ...
    if vanilla_generation:
        ...
        default_stop_strings = ["User:", "Question:", " #", "<|eot_id|>", "\t"]
        ...
        # Revert to skip_special_tokens=True as per the stable March 24 version
        txt = [tok.decode(x, skip_special_tokens=True) for x in gen_tokens.detach().cpu().numpy().tolist()]
        txt = [
