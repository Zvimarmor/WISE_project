...
import numpy as np
import scipy
# NLTK is only used for n_gram_entropy (fluency metrics), which WISE doesn't use
# Make it optional to avoid SQLite3 dependency issues
try:
    import nltk
    NLTK_AVAILABLE = True
except (ImportError, ModuleNotFoundError):
    NLTK_AVAILABLE = False
    print("Warning: NLTK not available. Fluency metrics will be disabled.")
import typing
...
def test_generation_quality(
    ...
    for name, module in model.named_modules():
        if hasattr(module, 'sticky_active'):
            ...
            # print(f"🧹 Reset sticky_active flag for {name}")
...
def compute_freq(sentence, n=2):
    if not NLTK_AVAILABLE:
        raise RuntimeError("NLTK is required for n-gram frequency computation but is not available")
