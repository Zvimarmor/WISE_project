...
class BaseEditor:
    ...
    def __init__(self, hparams: HyperParams):
        ...
        if not hparams.model_parallel and hasattr(hparams, 'device') and hparams.alg_name != 'QLoRA' and not getattr(self.model, 'is_loaded_in_8bit', False):
    ...
    def edit(self,
        ...
        # Inject sequential_edit flag into hparams for algorithms that need it (like WISE)
        if hasattr(self.hparams, 'sequential_edit'):
             self.hparams.sequential_edit = sequential_edit
        else:
             setattr(self.hparams, 'sequential_edit', sequential_edit)
    ...
    def edit_requests(self,
        ...
        getattr(self, 'first_edit_history', []) # Initialize safely
        """
        `prompts`: list or str
            the prompts to edit
        `ground_truth`: str
        ...
        def edit_func(request):
            if self.alg_name == 'IKE' or self.alg_name == 'ICE':
                ...
                if len(ret) == 3:
                    edited_model, weights_copy, loss_history = ret
                else:
                    edited_model, weights_copy = ret
                    loss_history = None
                icl_examples = None
            return edited_model, weights_copy, icl_examples, loss_history
        ...
        if sequential_edit:
            ...
            first_edit_request = requests[0]
            first_edit_history = []
            all_loss_history = []
            
            for i, request in enumerate(tqdm(requests, total=len(requests))):
                start_edit = time()
                edited_model, weights_copy, icl_examples, loss_history = edit_func(request)
                exec_time = time() - start_edit
                if verbose:
                    LOG.info(f"Edit {i} took {exec_time:.2f}s")
                
                if loss_history is not None:
                    # Append as dict: {'step': i, 'loss': [list of losses]}
                    all_loss_history.append({'step': i, 'loss': loss_history})
                    # Save loss history periodically (e.g., every edit or every 10)
                    # Saving every edit is safe for restartability
                    save_dir = getattr(self.hparams, 'save_path', None) or 'results'
                    os.makedirs(save_dir, exist_ok=True)
                    with open(os.path.join(save_dir, 'wise_loss.json'), 'w') as f:
                        json.dump(all_loss_history, f)

                # --- Save Model Weight Checkpoint (Every 50 edits) ---
                if self.alg_name == 'WISE' and i > 0 and i % 50 == 0:
                    if hasattr(self.hparams, 'save_path') and self.hparams.save_path:
                        print(f"Saving checkpoint at edit {i} to {self.hparams.save_path}")
                        self.model.save(self.hparams.save_path)
                # -----------------------------------------------------

                # --- Track First Edit Performance - New code we added with frequency check! ---
                if kwargs.get('track_first_edit', False):
                    eval_steps = kwargs.get('eval_steps', 1)
                    if i % eval_steps == 0:
                        start_eval = time()
                        # Evaluate the *first* request on the *current* model state
                        ...
                        # For temporal/hallucination, we care about 'post'. 
                        # Extract the relevant metric (e.g., probability/perplexity)
                        entry = {'step': i, 'metric': metrics['post'] if 'post' in metrics else metrics}
                        first_edit_history.append(entry)
                        
                        # --- Save Results Checkpoint ---
                        save_dir = getattr(self.hparams, 'save_path', None) or 'results'
                        os.makedirs(save_dir, exist_ok=True)
                        with open(os.path.join(save_dir, 'wise_results.json'), 'w') as f:
                            json.dump(first_edit_history, f)
                        # -------------------------------

                        eval_time = time() - start_eval
                        if verbose:
                            LOG.info(f"First Edit Eval at step {i} took {eval_time:.2f}s")
                # ------------------------------------

                # --- NEW: Evaluate IMMEDIATELY after each edit! ---
                print(f"Evaluating edit {i} immediately...")
        ...
        return all_metrics, edited_model, weights_copy, locals().get('first_edit_history', [])
