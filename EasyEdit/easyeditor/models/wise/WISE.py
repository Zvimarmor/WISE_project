...
class WISE(torch.nn.Module):
        ...
        if not self.config.retrieve:
            ...
            if hasattr(adapter, 'editing') and not adapter.editing:
                ...
                if len(adapter.memory_weight) > 0 and adapter.editing_total_cnt >= self.config.save_freq:
                    print('length of memory is ', len(adapter.memory_weight), '!!!!!!', flush=True)
    ...
    def get_adapter_layer(self):
        ...
        return adapter_layer.to(self.model.device) #return the adapter layer
    ...
    def edit(self, config, tokens, act_mask=None, deact_mask=None):
        ...
        if self.config.save_freq is not None and editing_total_cnt % self.config.save_freq == 0:
            ...
            print(f'Add New Weight to Memory...', flush=True)
        if editing_total_cnt % self.config.merge_freq == 0:
            ...
            print(f'Merge Weight of (New, Original) Matrix... with {self.config.merge_alg}', flush=True)
...
class WISEAdapter(torch.nn.Module):
    ...
    def merge_weight(self):
        if self.config.save_freq is not None:  # for ties dare dare_ties
            if not self.config.retrieve:
                ...
                print(len(self.memory_weight), flush=True)
            ...
            # Only update layer weight if not quantized
            if not (getattr(self.layer.weight, 'is_loaded_in_8bit', False) or self.layer.weight.dtype == torch.int8):
                cur_new_weight = merge_alg.execute(0.5, self.layer.weight, [self.new_weight],
                                                densities=self.config.densities)
                self.layer.weight = torch.nn.Parameter(cur_new_weight.to(self.layer.weight.device), requires_grad=False)
                
            self.new_weight = torch.nn.Parameter(self.original_layer.weight.clone())
    ...
    def forward(self, *args):
        if self.editing:
            ...
            # --- EVALUATION MODE ---
            # 1. Primary Sticky Check: Lock to the winner of the FIRST routed token
            if getattr(self.config, 'sticky_routing', False) and getattr(self, 'sticky_active', False):
                winner = getattr(self, 'sticky_winner', 'new')
                if winner == 'new':
                    # print(f"🔒 STICKY LOCK: Forcing Latest Edit", flush=True)
                    return self.new_weight_forward(*args)
                else:
                    # print(f"🔒 STICKY LOCK: Forcing Side-Memory {winner}", flush=True)
                    return self.layer(*args)

            # 2. Normal Routing Selection Logic
            if not self.config.retrieve:
            ...
            if id(layer_out) != id(original_layer_output):
                if getattr(self.config, 'sticky_routing', False):
                    ...
                    if id(layer_out) == id(new_weight_layer_output):
                        self.sticky_winner = 'new'
                    else:
                        self.sticky_winner = 'old'
                    print(f"🧲 STICKY TRIGGERED: Locking to {self.sticky_winner} memory for remainder of generation!", flush=True)
